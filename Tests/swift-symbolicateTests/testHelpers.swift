...
extension UsingLogStream {
  func getCrashLog(isJson: Bool, isSymbolicated: Bool, isMultiThreaded: Bool = false) async throws
    -> String
  {
    #if os(Windows)
    ...
    let appName: FilePath =
      if isMultiThreaded { FilePath("./.build/debug/crashMeMultithreaded") } else {
        FilePath("./.build/debug/crashMe")
      }
    ...
    if isJson,
      let log = crashRun.standardOutput.split(separator: "\n").first(where: { $0.hasPrefix("{") })
    {
      return String(log)
    } else {
...
extension UsingLogStream {
  ...
  func testScan(
    sampleData sampleDataString: String,
    process: (Data) throws -> Data
  ) async throws -> String? {
    ...
    let outputData = try #require(
      outputStream.property(forKey: .dataWrittenToMemoryStreamKey) as? Data)
  ...
  func symbolicateJsonCrashLog(
    ...
    options: BacktraceJSONFormatterOptions? = nil
  ) throws -> Data {
