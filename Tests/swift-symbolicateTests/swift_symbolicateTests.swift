...
typealias HostCrashLog = CrashLog<HostContext.Address>
  ...
  struct Utilities {
    @Test func testConverters() async throws {
      ...
      #expect(HostCrashLog.addressFromString("0x000000010455c000") == 0x0000_0001_0455_c000)
      #expect(HostCrashLog.addressFromString("0x000000019d24ff64") == 0x0000_0001_9d24_ff64)
    }
  ...
  struct PlainTextCrashLog {
    ...
    @Test func simplePlainTextRead() async throws {
      let reader = PlainCrashLogReader<HostContext.Address>(
        plainCrashLog: unsymbolicatedPlainCrashLog)
      let crashLog = try #require(reader.parse(), "unable to parse plain crash log")
    ...
    @Test func simplePlainTextMultiThreadWithRegistersRead() async throws {
      let reader = PlainCrashLogReader<HostContext.Address>(
        plainCrashLog: plainCrashLogSymbolicatedAllRegistersMultithreaded)
      let crashLog = try #require(reader.parse(), "unable to parse plain crash log")
      ...
      #expect(
        firstImage.path
          == "/Users/carlpeto/Code/swift-symbolicate/.build/arm64-apple-macosx/debug/crashMeMultithreaded"
      )
  ...
  struct JsonCrashLog {
    ...
    #if arch(arm64)
      @Test func simpleJsonRoundtripArm64() async throws {
  ...
  struct StreamScanning: UsingLogStream {
    @Test func recognizerTests() async throws {
      ...
      let jsonRecognizer = Recognizer(
        .init("{ \"timestamp\": \""), .init(skipTo: "\"", max: 100)!,
        .init("\", \"kind\": \"crashReport\""))
      // basic check
      #expect(checkRecognized(recognizer: jsonRecognizer, text: recognizerJsonTest) == .complete)
      // check it's reusable
      #expect(checkRecognized(recognizer: jsonRecognizer, text: recognizerJsonTest) == .complete)
      // more realistic check
      #expect(
        checkRecognized(
          recognizer: jsonRecognizer, text: unprettifiedUnsymbolicatedJsonTraceForRecognizer)
          == .complete)
      // failure check
      #expect(checkRecognized(recognizer: jsonRecognizer, text: recognizerJsonTest2) == .failed)
      // passthrough check
      #expect(checkRecognized(recognizer: jsonRecognizer, text: recognizerJsonTest3) == .noMatch)
      ...
      let jsonRecognizerTooShortSkip = Recognizer(
        .init("{ \"timestamp\": \""), .init(skipTo: "\"", max: 3)!,
        .init("\", \"kind\": \"crashReport\""))
      #expect(
        checkRecognized(recognizer: jsonRecognizerTooShortSkip, text: recognizerJsonTest) == .failed
      )  // basic check

      let jsonEndRecognizer = Recognizer(
        .init("\"backtraceTime\":"), .init(skipTo: "}", max: 100)!, .init("}"))
      // basic check
      #expect(
        checkRecognized(recognizer: jsonEndRecognizer, text: recognizerJsonEndTest) == .complete)

      let plainRecognizer = Recognizer(.init(" Program crashed: "))
      // basic check
      #expect(checkRecognized(recognizer: plainRecognizer, text: plainTextCrashLog) == .complete)

      let plainEndRecognizer = Recognizer(
        .init("Backtrace took "), .init(skipTo: "s", max: 100)!, .init("s"))
      // basic check
      #expect(
        checkRecognized(recognizer: plainEndRecognizer, text: plainTextCrashLogEnd) == .complete)
      // basic check
      #expect(
        checkRecognized(recognizer: plainEndRecognizer, text: plainTextCrashLogEnd) == .complete)
      // basic check
      #expect(checkRecognized(recognizer: plainEndRecognizer, text: plainTextCrashLog) == .complete)

    }

    @Test func testJsonCrashDumpCorrupted() async throws {
      // pass through unchanged
      let result = try await testScan(sampleData: jsonCrashLogCorruptedWithStartFinish) {
        crashDump in
    ...
    @Test func testJsonCrashDumpCorruptedWithFalseStart() async throws {
      // pass through unchanged
      let result = try await testScan(sampleData: jsonCrashLogCorruptedWithFalseStart) {
        crashDump in
    ...
    @Test func testJsonCrashDumpWithoutSymbolication() async throws {
      // pass through unchanged
      let result = try await testScan(sampleData: unprettifiedUnsymbolicatedJsonTraceForRecognizer)
      { crashDump in
        return crashDump
      }
  ...
  struct Symbolication: UsingLogStream {
    @Test func symbolicateJsonBacktrace() async throws {
      let unsymbolicatedJsonCrashLogText = try await getCrashLog(
        isJson: true, isSymbolicated: false)
      ...
      let crashedThreadBacktrace = crashedThread.backtrace(
        architecture: crashLog.architecture, images: images)

      let crashedThreadSymbolicatedBacktrace = try #require(
        crashedThreadBacktrace.symbolicated(
          with: images, options: [.showInlineFrames, .showSourceLocations])
      )

      #expect(crashedThreadSymbolicatedBacktrace.images.first?.name == "crashMe")
      #expect(
        crashedThreadSymbolicatedBacktrace.images.first?.baseAddress.description.hasPrefix("0x")
          == true)
    ...
    @Test func symbolicatePlainTextBacktrace() async throws {
      ...
      let crashedThreadBacktrace = crashedThread.backtrace(
        architecture: crashLog.architecture, images: images)
      let crashedThreadSymbolicatedBacktrace = try #require(
        crashedThreadBacktrace.symbolicated(with: images))
      crashedThread.updateWithBacktrace(symbolicatedBacktrace: crashedThreadSymbolicatedBacktrace)
  ...
  struct StreamSymbolication: UsingLogStream {
    ...
    @Test func testJsonCrashDumpWithSymbolication() async throws {
      let unprettifiedUnsymbolicatedJsonTrace = try await getCrashLog(
        isJson: true, isSymbolicated: false)

      #expect(!unprettifiedUnsymbolicatedJsonTrace.contains("s7crashMe6level4yyF"))

      let result = try #require(
        try await testScan(sampleData: unprettifiedUnsymbolicatedJsonTrace) { crashDump in
    ...
    @Test func testJsonCrashDumpWithSymbolicationIncludingExtraText() async throws {
      ...
      let result = try #require(
        try await testScan(sampleData: unprettifiedUnsymbolicatedJsonTraceWithBeforeAndAfter) {
          crashDump in
          try symbolicateJsonCrashLog(data: crashDump)
        })
    ...
    @Test func testJsonCrashDumpWithSymbolicationMultiThread() async throws {
      let jsonCrashLog = try await getCrashLog(
        isJson: true, isSymbolicated: false, isMultiThreaded: true)
      ...
      #expect(
        !jsonMultiThread.contains("backtraces will be missing information"),
        "The backtrace seems incomplete")

      let result = try #require(
        try await testScan(sampleData: jsonMultiThread) { crashDump in
    ...
    @Test func testPlainTextCrashDumpWithSymbolicationMulitpleThreads() async throws {
      let plainCrashLogMultithreaded = try await getCrashLog(
        isJson: false,
      ...
      #expect(
        !plainCrashLogMultithreaded.contains("backtraces will be missing information"),
        "The backtrace seems incomplete")
  ...
  struct EdgeCasesAndFuzzing: UsingLogStream {
    ...
    @Test func testJsonCrashDumpWithSymbolicationLargeBuffer() async throws {
      ...
      // pass through unchanged
      let result = try #require(
        try await testScan(sampleData: unsymbolicatedJsonLargeBuffer) { crashDump in
